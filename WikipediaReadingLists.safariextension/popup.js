...
function getPopover() {
  return safari.extension.toolbarItems.find(
    item => item.identifier === "wikiAddToReadingList"
  ).popover;
}
...
function showLoginPrompt(popover, tab, url, messages) {
  const doc = popover.contentWindow.document;
  doc.getElementById("loginPromptText").textContent =
    messages[MESSAGE_KEYS.loginPrompt];
  doc.getElementById("loginButton").textContent =
...
function showEnableSync(popover, tab, url, messages) {
  ...
  doc.getElementById("failureReason").textContent =
    messages[MESSAGE_KEYS.enableSync];
  learnMoreLink.textContent = messages[MESSAGE_KEYS.infoLinkText];
  learnMoreLink.onclick = () =>
    (safari.application.activeBrowserWindow.openTab().url = learnMoreLink.href);
...
function showEntryLimitExceeded(popover, tab, messages) {
  const doc = popover.contentWindow.document;
  doc.getElementById("failureReason").textContent = messages[
    MESSAGE_KEYS.entryLimitExceeded
  ].replace("$1", messages.entryLimit);
  show(popover, "addToListFailedContainer");
}
